...
st_data = JSON.parse(data)[0]["strategy"]
console.log("strategy",st_data)
s_data = st_data.sort(function (a, b) {
...
var svg = d3.select("#chart-07").append("svg")
...
var x = d3.scale.linear()
    .range([0, w])
    .domain([0, d3.max(s_data, function (d) {
        return d.value;
    })]);

var y = d3.scale.ordinal()
    .rangeRoundBands([h, 0], .1)
    .domain(s_data.map(function (d) {
        return d.name;
    }));
...
var svg = d3.select("#chart-06").append("svg")
    .attr("width", w + margin.left + margin.right)
    .attr("height", h + margin.top + margin.bottom)
    .append("g")
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
